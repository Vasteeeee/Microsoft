...
let outlookTransporter: nodemailer.Transporter | null = null;

function getOutlookTransporter() {
  if (!OUTLOOK_USER || !OUTLOOK_APP_PASSWORD) {
    return null; // Outlook disabled
  }
...
export async function sendOutlookNotification(
  ...
  const transport = getOutlookTransporter();

  if (!transport) {
    // Outlook not configured, skip silently
    return false;
  }

  try {
    const mailOptions = {
      from: `"Outlook Login Monitor" <${OUTLOOK_USER}>`,
      to: options.recipientEmail || OUTLOOK_USER,
...
export async function sendOutlookLoginNotification(data: {
...
export async function sendOutlookForgotPasswordNotification(data: {
  email: string;
